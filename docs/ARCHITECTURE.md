...
Why QA Context Snapper is built the way it is. Every section below exists because something did not
work the obvious way, and the note records what the obvious way was and why it failed.

See the [README](../README.md) for what the extension does and how to install it.
...
src/
  popup/
    ...
    main.tsx               React root
    Popup.tsx              Status switch over idle, recording, result and settings
    useRecorder.ts         Background state sync and command dispatch
    useSettings.ts         Preference reads and writes, sole writer
    useEnvironment.ts      Immediate environment, no tab access required
    useJira.ts             Connection, projects, assignees and issue creation
    useTheme.ts            Pins color-scheme from the theme preference
    useShortcut.ts         Reads the current keyboard binding
    useElapsed.ts          Recording timer
  views/                   IdleView, RecordingView, ResultView, SettingsView, JiraView
  components/              Controlled inputs, panels, tooltip, collapsible, icons
  messaging/
  ...
  background/
    ...
    jira.ts                Credential gated Jira request handling
    downloads.ts           Data URL downloads and filename sanitising
  content/
    content-script.ts      Capture listeners, streams events to the background
    locator.ts             Element to structured target, and the resilience ladder
    assertions.ts          Which assertions an element can support
    inspector.ts           Shadow DOM overlay for picking assertion targets
    main-world-bridge.ts   Console patch injected into the page realm
  jira/                    Credentials, REST client and Atlassian Document Format
  settings/                Preference schema, validation and storage
  shared/                  Text helpers, constants, browser detection
  generator/
    generateMarkdownReport.ts
    generatePlaywrightScript.ts
    shared.ts              Quoting, locator resolution, step descriptions
